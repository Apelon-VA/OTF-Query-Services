...
{
	private WhitespaceAnalyzer wa = new WhitespaceAnalyzer(LuceneIndexer.luceneVersion);
	private StandardAnalyzer sa = new StandardAnalyzer(LuceneIndexer.luceneVersion);
	...
	protected TokenStreamComponents wrapComponents(String fieldName, TokenStreamComponents components)
	{
		if (fieldName.endsWith(WHITE_SPACE_FIELD_MARKER))
		{
			return new TokenStreamComponents(components.getTokenizer(), new LowerCaseFilter(LuceneIndexer.luceneVersion, components.getTokenStream()));
		}
		else
		{
			return components;
